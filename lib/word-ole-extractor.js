...
class WordOleExtractor {
  ...
  buildDocument() {
    ...
    if (this._boundaries.ccpFtn) {
      document.setFootnotes(this.getTextRange(start, start + this._boundaries.ccpFtn - 1));
      start += this._boundaries.ccpFtn;  
    }

    if (this._boundaries.ccpHdd) {
      document.setHeaders(this.getTextRange(start, start + this._boundaries.ccpHdd - 1));
      start += this._boundaries.ccpHdd;  
    }

    if (this._boundaries.ccpAtn) {
      document.setAnnotations(this.getTextRange(start, start + this._boundaries.ccpAtn - 1));
      start += this._boundaries.ccpAtn;  
    }

    if (this._boundaries.ccpEdn) {
      document.setEndnotes(this.getTextRange(start, start + this._boundaries.ccpEdn - 1));
