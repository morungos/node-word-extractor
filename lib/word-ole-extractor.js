...
const getPieceIndexByCP = (pieces, position) => {
  for (let i = 0; i < pieces.length; i++) {
    const piece = pieces[i];
    if (position <= piece.filePos + piece.endPosition) { 
      return i; 
    }
  }
};
...
class WordOleExtractor {
  ...
  markDeletedRange(start, end) {
    this.replaceSelectedRange(start, end, '\x00');
  }
  ...
  //       throw new Error("Can't find piece for offset: " + offset);
  //     if (offset < pieceEnd) 
  //       return i;
  //   }
  //   throw new Error("Can't find piece for offset: " + offset);
  ...
  writeParagraphProperties(buffer, tableBuffer) {
    ...
    for(let i = 0; i < plcBtePapxCount; i++) {
      ...
      //console.log((require('util')).inspect(papxFkpBlockBuffer.inspect(), {depth: null, maxArrayLength: 512}));
