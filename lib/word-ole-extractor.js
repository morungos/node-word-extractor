...
class WordOleExtractor {
   ...
   * 
   * @todo
   * Handle the deleted text ranges, which are retrieved separately from
   * the character styling table. These ranges should also be omitted from
  ...
  prepare() {
    ...
    while (true) {    // eslint-disable-line no-constant-condition
      if (d >= deletions.length) break;
      if (p >= pieces.length) break;
      const piece = pieces[p];
      const deletion = deletions[d];

      const pieceStart = piece.filePos;
      const pieceEnd = piece.filePos + piece.length;

      let start = deletion.start;
      let end = deletion.end;

      if (pieceEnd < start) { p++; continue; }
      if (pieceStart > end) { d++; continue; }

      if (pieceStart > start) { start = pieceStart; }
      if (pieceEnd < end) { end = pieceEnd; }

      const original = piece.text;
      const modified = 
        ((start == pieceStart) ? '' : original.slice(0, start - pieceStart)) +
        ''.padStart(end - start, '\x00') +
        ((end == pieceEnd) ? '' : original.slice(end - pieceEnd));
      piece.text = modified;

      if (end > pieceEnd) {
        // Deletion extends beyond this piece
        deletion.start = end;
        p++;
      } else {
        d++;
  ...
  writeCharacterProperties(buffer, tableBuffer) {
    ...
    for(let i = 0; i < plcBteChpxCount; i++) {
      ...
      for(let j = 0; j < crun; j++) {
        ...
        let k = 0;
        while (k < grpprl.length - 1) {
          const sprm = grpprl.readUInt16LE(k);
          const ispmd = sprm & 0x1f;
          const fspec = (sprm >> 9) & 0x01;    // eslint-disable-line no-unused-vars
          const sgc = (sprm >> 10) & 0x07;     // eslint-disable-line no-unused-vars
