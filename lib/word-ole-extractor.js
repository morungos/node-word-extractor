...
class WordOleExtractor {
  ...
  cleanAndReplace(string) {
    return clean(replace(string));
  }
  ...
  buildDocument() {
    ...
    document.setBody(this.cleanAndReplace(this.getTextRange(start, start + this._boundaries.ccpText)));
    start += this._boundaries.ccpText;

    if (this._boundaries.ccpFtn) {
      document.setFootnotes(this.cleanAndReplace(this.getTextRange(start, start + this._boundaries.ccpFtn - 1)));
      start += this._boundaries.ccpFtn;  
    }

    if (this._boundaries.ccpHdd) {
      document.setHeaders(this.cleanAndReplace(this.getTextRange(start, start + this._boundaries.ccpHdd - 1)));
      start += this._boundaries.ccpHdd;  
    }

    if (this._boundaries.ccpAtn) {
      document.setAnnotations(this.cleanAndReplace(this.getTextRange(start, start + this._boundaries.ccpAtn - 1)));
      start += this._boundaries.ccpAtn;  
    }

    if (this._boundaries.ccpEdn) {
      document.setEndnotes(this.cleanAndReplace(this.getTextRange(start, start + this._boundaries.ccpEdn - 1)));
