...
class WordOleExtractor {
  ...
  buildDocument() {
    ...
    document.setBody(clean(this.getTextRange(start, start + this._boundaries.ccpText)));
    start += this._boundaries.ccpText;

    if (this._boundaries.ccpFtn) {
      document.setFootnotes(clean(this.getTextRange(start, start + this._boundaries.ccpFtn - 1)));
      start += this._boundaries.ccpFtn;  
    }

    if (this._boundaries.ccpHdd) {
      document.setHeaders(clean(this.getTextRange(start, start + this._boundaries.ccpHdd - 1)));
      start += this._boundaries.ccpHdd;  
    }

    if (this._boundaries.ccpAtn) {
      document.setAnnotations(clean(this.getTextRange(start, start + this._boundaries.ccpAtn - 1)));
      start += this._boundaries.ccpAtn;  
    }

    if (this._boundaries.ccpEdn) {
      document.setEndnotes(clean(this.getTextRange(start, start + this._boundaries.ccpEdn - 1)));
  ...
  normalizeHeaders(buffer, tableBuffer) {
    ...
    for(let i = 1; i < plcHddCount; i++) {
      ...
      console.log(i, start, end, this.getTextRange(start, end));
      start = end;
    }
