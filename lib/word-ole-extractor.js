 ...
 */
function getTextRangeByFilePos(pieces, start, end) {
  ...
  for (let i = startPiece, end1 = endPiece; i <= end1; i++) {
    const piece = pieces[i];
    const xstart = i === startPiece ? start - piece.startFilePos : 0;
    const xend = i === endPiece ? end - piece.startFilePos : piece.endFilePos;
...
class WordOleExtractor {
  ...
  buildDocument() {
    ...
    document.setBody(clean(this.getTextRangeByCP(start, start + this._boundaries.ccpText)));
    start += this._boundaries.ccpText;

    if (this._boundaries.ccpFtn) {
      document.setFootnotes(clean(this.getTextRangeByCP(start, start + this._boundaries.ccpFtn - 1)));
      start += this._boundaries.ccpFtn;  
    }

    if (this._boundaries.ccpHdd) {
      document.setHeaders(clean(this.getTextRangeByCP(start, start + this._boundaries.ccpHdd - 1)));
      start += this._boundaries.ccpHdd;  
    }

    if (this._boundaries.ccpAtn) {
      document.setAnnotations(clean(this.getTextRangeByCP(start, start + this._boundaries.ccpAtn - 1)));
      start += this._boundaries.ccpAtn;  
    }

    if (this._boundaries.ccpEdn) {
      document.setEndnotes(clean(this.getTextRangeByCP(start, start + this._boundaries.ccpEdn - 1)));
  ...
  normalizeHeaders(buffer, tableBuffer) {
    ...
    const offset = this._boundaries.fcMin + this._boundaries.ccpText + this._boundaries.ccpFtn;
    ...
    for(let i = 1; i < plcHddCount; i++) {
      const end = offset + plcHdd.readUInt32LE(i * 4);
      //console.log(i, start, end, this.getTextRangeByCP(start, end));
    ...
    for(let i = 0; i < plcBtePapxCount; i++) {
      ...
      for(let j = 0; j < crun; j++) {
        ...
        processSprms(grpPrlAndIstd, 2, (buffer, offset, sprm, ispmd, fspec, sgc, spra) => {    // eslint-disable-line no-unused-vars
          ...
          if (sprm === 0x2417) {
            this.replaceSelectedRangeByFilePos(rgfc, rgfcNext, '\n');
          }
        });
      }
