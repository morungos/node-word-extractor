      ...
      .then((zipfile) => {
        return new Promise((resolve, reject) => {
          ...
          zipfile.on("end", () => {
            ...
            document.setBody(this._pieces.join(""));
            resolve(document);
          });  
        });
      });
  ...
  createXmlParser() {
    ...
    parser.on("opentag", (node) => {
      ...
      if (node.name === 'Override') {
        ...
        this._stack = [];
      } else if (node.name === 'w:tab' && this._context === 'document') {
        this._pieces.push("\t");
      } else if (node.name === 'w:br' && this._context === 'document') {
    ...
    parser.on('closetag', (node) => {
      if (node === 'w:document') {
        this._context = null;
      } else if (node === 'w:p' && this._context === 'document') {
        this._pieces.push("\n");
      } else if (node === 'w:p' && this._context === 'cell') {
        this._pieces.push("\t");
    ...
    parser.on('text', (string) => {
      if (this._context === 'document' || this._context === 'cell') {
