  ...
  createXmlParser() {
    ...
    parser.on("opentag", (node) => {
      ...
      if (node.name === 'Override') {
        ...
        this._inTabs = false;
        this._stack = [];
      } else if (node.name === 'w:tab' && this._inDocument && ! this._inTabs) {
        this._pieces.push("\t");
      } else if (node.name === 'w:br' && this._inDocument) {
        ...
        this._stack.push(this._inDocument);
        this._inDocument = false;
      } else if (node.name === 'w:tabs') {
        this._stack.push(this._inTabs);
        this._inTabs = true;
      }
    ...
    parser.on('closetag', (node) => {
      if (node === 'w:document') {
        ...
        this._inDocument = this._stack.pop();
      } else if (node === 'w:tabs') {
        this._inTabs = this._stack.pop();
      }
    });
