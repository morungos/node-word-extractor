...
class OpenOfficeExtractor {
  ...
  createXmlParser() {
    ...
    parser.on("opentag", (node) => {
      ...
      if (node.name === 'Override') {
      ...
      } else if (node.name === 'w:document' || node.name === 'w:footnotes' || node.name === 'w:endnotes') {
        this._context = 'content';
        this._stack = [];
        this._pieces = [];
      } else if (node.name === 'w:tab' && this._context === 'content') {
        this._pieces.push("\t");
      } else if (node.name === 'w:br' && this._context === 'content') {
        ...
        this._stack.push(this._context);
        this._context = 'deleted';
    ...
    parser.on('closetag', (node) => {
      if (node === 'w:document') {
        this._context = null;
        this._document.setBody(this._pieces.join(""));
      } else if (node === 'w:footnotes') {
        this._context = null;
        this._document.setFootnotes(this._pieces.join(""));
      } else if (node === 'w:endnotes') {
        this._context = null;
        this._document.setEndnotes(this._pieces.join(""));
      } else if (node === 'w:p') {
        this._pieces.push("\n");
      } else if (node === 'w:del' || node === 'w:instrText') {
        this._context = this._stack.pop();
      } else if (node === 'w:tabs') {
        this._context = this._stack.pop();
      } else if (node === 'w:tc') {
        this._pieces.pop();
    ...
    parser.on('text', (string) => {
      if (this._context === 'content' || this._context === 'cell') {
